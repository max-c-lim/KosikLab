...
recordings_path = Path("/home/maxlim/SpikeSorting/data/DL/recordings/maxone_2950.raw.h5")
inter_path = Path("/home/maxlim/SpikeSorting/data/DL/sorted/processed/alessio/maxone_2950")
...
########################################################################################################################
# Let's do it!
print(f"Processing: {recordings_path}")
...
if str(recordings_path).endswith(".raw.h5"):
    recording = se.MaxOneRecordingExtractor(recordings_path, load_spikes=False)
else:
    ...
    experiment_names = natsorted([p.name for p in recordings_path.iterdir() if p.name.endswith(".raw.h5")])

    for exp in experiment_names:
        rec_file = [p for p in recordings_path.iterdir() if p.name == exp][0]
        rec = se.MaxOneRecordingExtractor(rec_file, load_spikes=False)
...
if auto_curate:
    if (sorting_cache / 'sorting_curated.pkl').is_file() and not recompute_curation:
        ...
        #
        if False:  # export_curated_to_phy
