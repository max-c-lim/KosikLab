...
recording_files = [
    "/home/maxlim/SpikeSorting/data/maxone/200123/2950/recordings",
    # r"E:\KosikLab\SpikeSorting\data\maxone\recordings\control\control.raw.h5"
]

# List of intermediate folders where tmp and output files are saved
intermediate_folders = [
    # "E:/KosikLab/SpikeSorting/data/maxone/test/processed"
    "/home/maxlim/SpikeSorting/data/maxone/200123/2950/concatenated/processed"
]

# List of output folders where final matlab files are saved.
# Matlab files will have same name as recording files but will end with _sorted.mat
matlab_folders = [
    "/home/maxlim/SpikeSorting/data/maxone/200123/2950/concatenated"
]
...
kilosort_params = {
    ...
    'total_memory': "64G",  # Too low memory will cause error
    'n_jobs_bin': 16
}
...
rewrite_recording = False  # Refers to the .dat recording file created for Kilosort
...
class KilosortSortingExtractor:
    ...
    def __init__(self, folder_path, exclude_cluster_groups=None, keep_good_only=False):
        ...
        # Prevents unit_ids without any spikes from being included
        # self.unit_ids = []
        # spike_clusters_set = set(spike_clusters.flatten())
        # for unit_id in unit_ids:
        #     if unit_id in spike_clusters_set:
...
class Stopwatch:
    ...
    def log_time(self, text=None):
        if text is None:  # Backwards compatibility
...
def load_recording(rec_path):
    ...
    try:
        ...
        print("Done loading recording." + stopwatch.log_time())
...
def concatenate_recordings(rec_path):
    ...
    # Each recording is in a separate folder in rec_path
    recordings = []
    for recording_folder in rec_path.iterdir():
        for recording_file in recording_folder.iterdir():
            rec = se.MaxwellRecordingExtractor(recording_file)
            recordings.append(rec)
            print(f"{recording_folder.name}: DURATION: {rec.get_num_frames() / rec.get_sampling_frequency()} s -- "
...
def get_paths(rec_path, inter_path):
    ...
    if recompute_sorting and inter_path.exists():
        for path in inter_path.iterdir():
            if not str(path).endswith(".dat") or rewrite_recording:
                shutil.rmtree(path)
    ...
    # region Unused by script but need to run kilosort matlab files
...
def write_recording(recording, inter_path, verbose=False):
    ...
    if not recording_dat_path.exists():
        BinaryRecordingExtractor.write_recording(recording, file_paths=recording_dat_path,
                                                 dtype='int16', total_memory=total_memory,
                                                 n_jobs=n_jobs, verbose=False, progress_bar=verbose)
    else:
        print(f"Using existing {recording_dat_path} as recording file")
...
def curate_units(rec_cache, sorting, cache_folder):
...
def process_recording(rec_path, inter_path):
    ...
    # Filter and Dumping
    rec_cache, recording_dat_path = write_recording(rec, inter_path, verbose=True)

    # Spike Sorting
    sorting = spike_sort(rec_cache=rec_cache, rec_path=rec_path, recording_dat_path=recording_dat_path, output_folder=output_folder)
    if sorting == -1:  # Could not sort recording
        return "Could not sort recording. Moving on to next recording"

    # Curating Data
    waveform_extractor = curate_units(rec_cache, sorting, cache_folder)

    print(f"\n\nDone processing recording." + stopwatch.log_time())
